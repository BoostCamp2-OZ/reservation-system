...
<div id="container">
    ...
    <div class="ct">
        <div class="ct_wrap">
            <div class="top_title">
                <a href="/products/${product.id}" class="btn_back" title="이전 화면으로 이동"> <i class="fn fn-backward1"></i> </a>
                <h2><span class="title">${product.name}</span></h2>
            </div>
            <div class="group_visual">
                <div class="container_visual" style="width: 414px;">
                    <ul class="visual_img">
                        <li class="item" style="width: 414px;"> <img alt="" class="img_thumb" src="/files/${product.files[0].fileId}"> <span class="img_bg"></span>
                            <div class="preview_txt">
                                <h2 class="preview_txt_tit">${product.name}</h2>
                                <em class="preview_txt_dsc"></em><em class="preview_txt_dsc">
                                    <fmt:formatDate value="${product.displayStart}" pattern="YYYY-MM-DD"/>  ~ <fmt:formatDate value="${product.displayEnd}" pattern="YYYY-MM-DD"/>
                                </em>
                            </div>
                        </li>
                    </ul>
            ...
            <div class="section_store_details">
                <div class="store_details">
                    ...
                    <p class="dsc">
                        장소 : ${product.placeLot} (${product.placeStreet})<br>
                        기간 : <fmt:formatDate value="${product.displayStart}" pattern="YYYY-MM-DD"/>  ~ <fmt:formatDate value="${product.displayEnd}" pattern="YYYY-MM-DD"/>
                    </p>
                    <h3 class="in_tit">관람시간</h3>
                    <p class="dsc">
                        ${product.observationTime}<br>
                    </p>
                    ...
                    <c:forEach var="price" items="${product.prices }">
                        <c:choose>
                            <c:when test="${price.priceType eq 1 }">
                                ...
                                <c:set var="priceType" value="general_ticket_count"/>
                            </c:when>
                            <c:when test="${price.priceType eq 2 }">
                                <c:set var="age" value="청소년"/>
                                <c:set var="priceType" value="youth_ticket_count"/>
                            </c:when>
                            <c:when test="${price.priceType eq 3 }">
                                <c:set var="age" value="어린이"/>
                                <c:set var="priceType" value="child_ticket_count"/>
                            </c:when>
                        </c:choose>
                        <div class="qty" data-index="${num}">
                            <div class="count_control">
                                ...
                                <div class="clearfix">
                                    <a href="#" class="btn_plus_minus spr_book2 ico_minus3 disabled _minus" title="빼기"> </a> <input type="tel" class="count_control_input disabled" value="0" readonly title="수량">
                                    <a href="#" class="btn_plus_minus spr_book2 ico_plus3 _plus" title="더하기">
                                    </a>
                                </div>
                                <!-- [D] 금액이 0 이상이면 individual_price에 on_color 추가 -->
                                <div class="individual_price" data-price="${price.price*(1-price.discountRate)}"><span class="total_price">0</span><span class="price_type">원</span></div>
                            </div>
                            <div class="qty_info_icon"> <strong class="product_amount" >
                                <span data-type="${priceType}">${age}</span> </strong>
                        ...
                        <form class="form_horizontal">
                            ...
                            <div class="inline_form last"> <label class="label" for="message">예매내용</label>
                                <div class="inline_control">
                                    <p class="inline_txt selected"><fmt:formatDate value="${product.displayStart}" pattern="YYYY-MM-DD"/>  ~ <fmt:formatDate value="${product.displayEnd}" pattern="YYYY-MM-DD"/>, 총<span class="_totalTicketCount">0</span> 매</p>
                                </div>
                            </div>
                        </form>
                    </div>
            ...
            <div class="box_bk_btn">
                ...
                <div class="bk_btn_wrap disable" data-productid="${product.id}"> <button type="button" class="bk_btn"> <i class="spr_book ico_naver_s"></i>  <span>예약하기</span> </button> </div>
            </div>
        </div>
    </div>
</div>
...
<script src="/resources/dist/reserve.bundle.js"></script>
</html>
